...
$client = new nusoap_client($ws,true);
	...
	if ($sError) {
		...
		if($error){
				...
				$empresas = $client->call('ItsGetData', array('UserSession' => $session, 'ItsClassName' => 'ERP_EMPRESAS', 'RecordCount' => '100', 'SQLFilter'=>''  , 'SQLSort'=> '') );
					...
					if($ItsGetDataResult){
								...
								for ($i=0; $i<sizeof($langs); $i++) {
									$datos = array('ID'=>$langs[$i]['@attributes']['ID'],
													  ...
													  'NUM_DOC'=>$langs[$i]['@attributes']['NUM_DOC']
														);
									$salida[] = $datos;
								}
								echo json_encode(array("ItsLoginResult"=>$ItsGetDataResult, "ItsGetDate"=>$ItsGetDate, "Data"=>$salida));
